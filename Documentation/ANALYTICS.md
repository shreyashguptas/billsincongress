...
| `answer_failed` | Request errored (not rate limit) | `surface`, `error` — one of `"connection_failed"`, `"stream_dropped"` (connection cut mid-answer, the idle-timeout fingerprint), `"no_stream_body"`, `"stream_incomplete"`, or the server's message, `elapsed_ms` — ms waited before failing, `stream_started` — whether any byte arrived first | `components/answers/answer-provider.tsx` |
