...
| `alerts.ts`, `email.ts` | Bill alerts: follow/unfollow, the account list, token unsubscribe, the daily digest run; `email.deliverAlert` hands each digest to PostHog |
...
- schedules at most one email (`email.deliverAlert`) and advances the watermarks **in the same
  transaction**: a scheduled function exists only if its transaction commits, so a bill is
  never reported twice and never marked reported without an email queued. `deliverAlert`
...
4. PostHog: the "Bills.Congress: bill alerts" workflow exists (same sender and webhook secret as
   ...
   string).
5. Set `ALERT_EMAILS_LIVE=true` **only on production**. Without it `email.deliverAlert` logs
   and skips every send, so a dev deployment holding copies of real users can never mail them.
...
`email:deliverAlert`. Webhook history is in `stripeEvents`; a `failed` row is retried by
