  ...
  async storeBills(bills: Bill[]): Promise<void> {
    try {
      ...
      const billsToStore = await Promise.all(bills.map(async bill => {
        ...
        return {
          id: bill.id || `${bill.congress}-${bill.type?.toLowerCase()}-${bill.number}`,
          title: bill.title,
          congress_number: bill.congressNumber || bill.congress,
          bill_type: bill.billType || bill.type,
          bill_number: bill.billNumber || parseInt(bill.number || '0'),
          ...
          committee_count: bill.committeeCount || 0,
          latest_action_text: bill.latestActionText || bill.latestAction?.text || '',
          latest_action_date: bill.latestActionDate || bill.latestAction?.actionDate || '',
          update_date: bill.updateDate || '',
          status: bill.status || 'In Progress',
          ...
          congress_gov_url: bill.congressGovUrl || bill.url || '',
          ...
          official_title: bill.officialTitle || bill.title,
          short_title: bill.shortTitle || '',
          cosponsors_count: bill.cosponsorsCount || 0,
          bill_text_url: bill.billTextUrl || null,
          bill_pdf_url: bill.billPdfUrl || null,
          last_updated: new Date(bill.updateDateIncludingText || bill.updateDate || now).toISOString(),
