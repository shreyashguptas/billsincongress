# Agent setup for `billsincongress`

This is a deepsec scanning workspace. Project `billsincongress` was just registered
(target: `..`). Setup is incomplete — `data/billsincongress/INFO.md`
...
2. **Fill in `data/billsincongress/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id billsincongress
pnpm deepsec process --project-id billsincongress
```
