...
export const DATASETS: Record<DatasetName, DatasetDoc> = {
  bills: {
    ...
    gotchas: [
      ...
      "A measure with no sponsorParty has NO SPONSOR AT ALL on Congress.gov, not a sponsor whose party is unknown. The home page seats these as 'party not recorded'. They are bill numbers the House held back for its leadership, never written or acted on — read each row's title for whom it was reserved (it varies) and describe them that way, not as bills someone wrote. List them with {\"sponsorParty\": \"none\"} and read the count off that result; do not state one from this note.",
