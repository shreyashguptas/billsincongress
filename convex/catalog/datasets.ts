...
export const DATASETS: Record<DatasetName, DatasetDoc> = {
  bills: {
    ...
    fields: [
      ...
      { name: "sponsorParty", type: "string", meaning: "'D', 'R' or 'I'. Empty when no party is recorded — see gotchas." },
      ...
      { name: "finalStatus", type: "string", meaning: "Present only on an unfinished measure from a Congress that has ENDED: what became of it, in words. Repeat what it says, in the past tense — never 'unlikely to' or 'may still' — and claim no more than it does." },
    ...
    filters: [
      ...
      { name: "sponsorParty", type: "string", allowed: "'D', 'R', 'I', or 'none' for measures with no party recorded — what the home page calls 'party not recorded'.", example: "none" },
    ...
    gotchas: [
      ...
      "A measure with no sponsorParty has NO SPONSOR AT ALL on Congress.gov, not a sponsor whose party is unknown. The home page seats these as 'party not recorded'. They are bill numbers the House held back for its leadership: each has a TITLE saying whom it was reserved for (e.g. 'Reserved for the Minority Leader.'), and a reservedFor field with the same. Do not say they have no title or no action; read their dates and stage off the rows. List them with {\"sponsorParty\": \"none\"}. WHO got how many comes from the result's exact_subsets and nothing else: the numbers are not grouped by leader, and splitting them by eye has put a bill under the wrong leader. State no count from this note — only from a fetch. Say only what the rows show, not how such numbers are usually used.",
    ...
    examples: [
      ...
      '{ "congress": 117, "sponsorParty": "none" } — the measures the home page shows as party not recorded',
  ...
  stats: {
    ...
    fields: [
      ...
      { name: "partyCounts", type: "object", meaning: "Measures introduced by sponsor party: D, R, I, and U = any other value, shown on the home page as 'party not recorded'. Both chambers on the whole-Congress row, one chamber on a chamber row." },
      { name: "partyLawCounts", type: "object", meaning: "Measures that BECAME LAW, split the same way as partyCounts." },
