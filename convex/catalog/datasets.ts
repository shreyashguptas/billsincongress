...
export const DATASETS: Record<DatasetName, DatasetDoc> = {
  bills: {
    ...
    gotchas: [
      ...
      "A measure with no sponsorParty has NO SPONSOR AT ALL on Congress.gov, not a sponsor whose party is unknown. The home page seats these as 'party not recorded'. They are bill numbers the House held back for its leadership: each has a TITLE saying whom it was reserved for (e.g. 'Reserved for the Minority Leader.'), a reservedFor field with the same, and an introduction on the Congress's first day — then nothing further. Do not say they have no title or no action. List them with {\"sponsorParty\": \"none\"}. WHO got how many comes from the result's exact_subsets and nothing else: the numbers are not grouped by leader, and splitting them by eye has put a bill under the wrong leader. State no count from this note — only from a fetch. Say only what the rows show, not how such numbers are usually used.",
